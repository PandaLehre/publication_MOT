...
\begin{abstract}
	This study introduces Machine Learning (ML)-based methodologies for Multi-Object Tracking (MOT) tailored to Advanced Driver Assistance Systems (ADAS), addressing the growing complexity and precision requirements in the automotive sector.
...
%CH: changed "tracking" to "state predictions", because of the subsection.
...
%CH: changed "video material" to "data sequence".
...
%CH: changed the sentence to be more precise.
...
%CH: added sentence to clarify the data analysis is done in the next section.
...
%		Allerdings: kann auch eine definierte Anzahl an Schritten betrachtet werden.
%       (Beispiel: Input immer 10 Zeitschritte (Sensorzyklen), wird meines Wissens nach in vielen Perception Stacks so gemacht)
%       Über SPENT haben wir ein "zeitliches Tracking" und über SANT / MANTa ein "räumliches Tracking".
...
Figure {\ref{pic::MantaData}} shows the non-noisy sensor objects to enable a example assignment and increase understanding of the association procedure.
%MD: Die fig 5 zeigt aber gar kein assignment.
%CH: Stimmt, aber der one-hot Vektor in fig 5 zeigt das Assignment :) 
...
%MD: Die Bezeichnungen in der Abbildung und die im Text müssen exakt gleich sein.
%CH: Habe ich angepasst.
