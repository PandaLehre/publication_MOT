...
%MD: Warum parameter set? Einstellbare Parameter sind es ja nicht.
...
%CH: predicted states
where $R$ is the number of predicted states, $t_i$ is the ground truth value (from the Kitti tracks) and $y_i$ is the prediction of the network for sample $i$.
For our sequence-to-sequence regression network the loss function of the regression layer is half the mean square error of the predictions for each time step, normalized by the sequence length $S$, not by the number of the predicted Statevector $R$: %MD: Check if the word 
\begin{equation}
