...
in early timesteps of detection with a accurate prediction.
...
% CH: Kommentar von CB eingearbeitet
But from a software engineering point of view hand-engineered parameters are only available in
a limited amount of number and heuristics are extremely hard to maintain
...
considering the current state of the tracks and the sensor objects.
...
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.95\linewidth]{figs/padding.png}}
...
The size of $OH_{max}=288$ results from the maximum number of tracks $T_{max}=16$ and the number of possible assignment $classes = 18$.
