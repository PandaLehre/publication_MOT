...
In complex and dynamic environments, as encountered in urban traffic, it is crucial to accurately capture and predict the positions of multiple objects already in the early time stamps of detection -- a key challenge in assisted and automated driving.
...
%MD: Minor points (würde ich erstmal lassen): X sollte wahrscheinlich jetzt zu T^p umbenannt werden für Tracks predicted. Auch finde ich es komisch, dass vor und nach der Prediction das t gleich bleibt. Dafür würde ich eigentlich bei T^c oben t-1 schreiben. Allerdings müssen alle Bezeichnungen in den Formeln konsistent mit denen hier sein. UND das müsste auch in Fig. 3 geändert werden. Also lassen wir das jetzt mal. UND in Fig. 6. Zu viele Änderungen.
%MD: In der Grafik sprechen wir noch von "timesteps".
\begin{figure}[!hbp]
...
The "Association network" can be implemented using either \ac{SANT} or \ac{MANTa}. It works in tandem with the prediction network \ac{SPENT}, which takes tracked objects ${T^c}_{t,1:n}$ and predicts them to the next time stamp as $X_{t,1:n}$. The predicted objects are then associated with sensor observations $Z_{t,1:m}$ by \ac{SANT} or \ac{MANTa}.
}
...
The input for the proposed prediction network (i) are up to $m$ \acp{SO} $Z_{t,1:m}$ at time stamp $t$.
...
\ac{SPENT} predicts all currently tracked objects $X_{t,1:n}$ (up to $n$) to the next time stamp where they are used as input to either the SANT or MANTa association networks.
...
The mean values $\vec{\mu}$ and standard deviations $\vec{\sigma}$ for each state in the state vectors $\vec{Z}_t$ were computed across all tracks. For this, all tracks were joined together, leading to one pseudo-track with a total number of time stamps $N$:
...
\end{equation}
where $Z_{t,i}$ refers to the $i$-th entry in the state vector at time stamp $t$.
...
To generate the synthetical \ac{GT}, we take a set of tracks, then randomly chose one of them and take the next state vector at the next time stamp of that single track as the incoming new \ac{SO}. %MD: Wenn wir das noch einen Time stamp nach vorne predicten, dann haben die anderen Objekte und das rausgesuchte zum SO erklärte ja nicht mehr den gleichen time stamp. Wie kann das sein?
...
Fig. {\ref{pic::MantaData}} shows the input data structure with corresponding association tasks for time stamp 85 of sequence 20 of the KITTI data set. Equivalent data were extracted across all sequences.
...
The ordering of the \ac{SO}s is randomized per time stamp and the resulting association input matrix has dimension $F_{total} \times T_{\max}$, where $F_{total}$ represents the total number of features (here $2 \times k=10$, with $k$ being the number of values per state vector).
...
\ac{SPENT} substitutes the state predictions of the Kalman Filter by directly estimating predictions per time stamp, eliminating the need for a predefined dynamics model.
...
\end{table}
Our recurrent network updates its internal hidden states at each time stamp, capturing temporal dependencies and enabling accurate state predictions without external correction.
...
%MD: Im Text schreibst du MANTa schafft 70% über alles. Hier steht aber 70% für 7-16. Eins muss falsch sein.
\begin{table}[b]
    \centering
...
Expanding on \ac{SANT}, \ac{MANTa} addresses the limitations of a single object to track assignment by enabling multi object assignments within each time stamp.
This means \ac{MANTa} is trained to assign a list of \acp{SO} to a list of tracks in a single operational step on the same dataset as \ac{SANT} and also achieved an assignment accuracy of 95\% for the six most frequently occurring association sets, i.e., the sets with one to six tracks per time stamp. It performs worse for assignment scenarios with more tracks, which were less present in the training data.
...
As illustrated in Fig. {\ref{pic::MantaTracks}}, the distribution of the numberof existing tracks per time stamp reveals significant insights.

Notably, time stamps containing exactly one track constitute nearly one-third of the entire dataset, accounting for 29.9\% of the data, which corresponds to an absolute count of 2315 samples.
...
% In this paper, we present  machine learning methodologies that significantly advance multi-object tracking (MOT) tailored for the demands of advanced driver assistance systems (ADAS). Our approach integrates three innovative neural network models—SPENT, SANT, and MANTa—within a traditional Kalman Filter (KF) framework, enhancing both modularity and performance without disrupting its architecture.

% Our evaluation on the KITTI dataset demonstrated substantial improvements in tracking accuracy and robustness. The SPENT model achieved a root mean square error (RMSE) of 0.029 for positional predictions, outperforming the standard KF by reducing the RMSE by over 50%. Furthermore, the SANT and MANTa models achieved an impressive 95% accuracy in sensor object-to-track assignments, highlighting the effectiveness of data-driven approaches in resolving complex association tasks.

% By replacing heuristic-based components with task-specific neural networks, our framework preserves the interpretability and modularity of traditional tracking systems while harnessing the adaptability and precision of machine learning.

% Looking ahead, this work lays a robust foundation for future research and development in ADAS tracking systems, emphasizing the potential of integrating machine learning models to meet the evolving challenges of automated driving. A promising direction for future work includes the exploration of uncertainty quantification, which can enhance decision-making processes by providing confidence levels in tracking predictions, thereby further improving system reliability and safety in real-world applications. Additionally, investigating multi-tasking networks could optimize the context-aware capabilities of multi-object tracking at the object level, allowing for more nuanced and efficient tracking solutions that consider the specific characteristics of each tracked object.
