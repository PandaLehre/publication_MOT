...
\subsection{Association Network (Single Association Network)}
Das Assoziationsnetzwerk stellt einen datenbasierten Ansatz dar, welcher lernen kann, das kombinatorische Non Deterministic Polynomial Time (NP) hard Optimierungsproblem der Datenassoziation vollständig zu lösen.
...
Somit ersetzt das Assoziationsnetzwerk die Berechnung eines Abstandsmaßes sowie einen Assoziationsalgorithmus wie z.B. den Hungarian Algorithm (HA) \cite{DA_hungarian.1955}.

\paragraph{Datenvorverarbeitung}
Für die Entwicklung eines Single Association Networks (SANT) wurde die Datenassoziation als zeitlich gegliedertes Problem (Sequenz-zu-Vektor) betrachtet.
Das Datenassoziationsproblem beschreibt somit die Zuordnung von genau einem SO zu einem Set von Tracks.
...
Pro Zeitschritt wurden alle existierenden GT Tracks einzeln verrauscht.
Dafür wurde jedem Wert des aktuellen Zustandsvektors maximal 3\% zufällig abgezogen oder dazu addiert. 
Der Datensatz wurde in 7 Iterationen erstellt, sodass pro Iteration die Rauschintensität um 0.5\% gesteigert wurde.
...
Die Größe der Inputmatrix entspricht daher $m * n + 1$. 
Mit $m = Anzahl der Zustandswerte$ und $n = Trackanzahl(t)$.
