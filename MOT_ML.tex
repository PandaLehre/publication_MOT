...
In classical systems that do not learn directly from data, these issues are typically addressed through 
the implementation of heuristics. Additionally, parameters of the tracking system may be 
adjusted to accommodate the problematic scenes.
...
% ME: We do this preserving one of the biggest strengths of classical approaches: its modularity, by replacing only single tracking components with ML models.
We aim to preserve one of the biggest strengths of classical approaches, i.e. their modularity, and replace only individual tracking components with ML-based models:
\menz{We should streamline the following and explicitly state the contributions so that the reviewers understand this. 
We should also mention the method abbreviations from the abstract, e.g.: Our contributions are: (i) We propose SPENT, a prediction network that is ..... (ii) SANT and MANTa .... 
(iii) We present a thorough evaluation by integrating our models into an existing tracking system ..... }
(i) In contrast to the Kalman filter {\cite{KF_BoundingBD.2023}}, our proposed prediction network is capable of predicting the state of individual objects without the need for a predefined state or prediction model at runtime.
...
(ii) Our proposed Single and (iii) Multi Association Network replaces the calculation of a distance metric for the corresponding assignment by employing machine learning in order to resolve situations unclear for traditional approaches.
...
One fundamental problem in Tracking-by-Detection (TbD) frameworks is the prediction of the states of the already tracked objects.
...
In the context of this work, we put forward the hypothesis that a Gated Recurrent Unit (GRU)-based association network can be designed and trained using an undefined distance measure, as discussed in the next chapter.
...
\menz{Do you think we could integrate this section into the contributions section in the intro? This would also help to get a figure on the first page.
There has been an interesting CVPR paper some years ago that classified papers into accepted / non-accepted. Having a figure on the first page, was the
major factor to have a paper accepted :)}
\menz{If we do not want to remove this section, then I would still argue for Fig. 1 (or another figure) on the first page }
...
\begin{figure}[htbp]
	...
	\menz{Sensorobjects should be sensor objects in the legend; same for hiddenstate; and the fonts look weird on my machine. If we decided to put this figure,
	on the first page, we would need a more elaborate caption. One should be able to fully understand the approach by looking at the figure and 
	reading the caption. }}
...
\menz{This section is a bit unconventional because it integrates both the technical approach and the experimental results in a single section vs.
having a dedicated section on experimental evaluation. What's the reason for that? Wouldn't it make more sense, to keep that separate?
Particularly the last paragraph in IV-A would fit much better into an experimental evaluation section. There is no way to put the reported numbers there 
in context at this point in the paper.}
\menz{I'd suggest to move all the dataset (preprocessing) and experimental results into a dedicated section. It should be possible to explain the model 
archictures without knowing much of the details of the particular dataset used. }
...
This was used accordingly in this study. \menz{How?}
To enable our models to incorporate temporal information, we use LSTM and bidirectional Long Short-Term Memory (BILSTM) networks \menz{references ?}, both for the 
prediction and the association of the sensor objects to the existing tracks.
...
\menz{There are some different capitalizations of association network / Association Network and prediction / Prediction Network throughout the text. Need to check for consistency. 
A good way is to introduce a LaTeX \textbackslash newcommand\{prednet\}\{Prediction Network\} and use this instead of typing it out.}
...
The track lengths vary from a minimum of 4 frames to a maximum of 643 frames (Sequence 20, ID 12). \menz{What's the motivation for that preprocessing? Why can't the data be 
used as is? I think we need to explain this in one sentence. Otherwise it sounds a bit like if the ground truth was specifically tuned to make our approach look better.}
...
\begin{figure}[htbp]
	...
	\caption{Analysis of Sequence Padding: Unsorted vs. Sorted Data.
			...
			to batching significantly curtails the requisite padding. \menz{Nice figure and caption!!}}
	\label{pic::Padding}
\end{figure}
...
The association network facilitates a data-driven methodology to address the combinatorial optimization challenge of data association, which is known to be NP-hard \menz{is it? needs a reference}.
...
% ME: Added some ideas for an experimental evalation section
\section{Experimental evaluation}
\begin{itemize}
	\item introduce the dataset
	\item not sure if the specific data preprocessing should be here or in the previous section \dots
	\item explain the metrics and evaluation methodology
	\item experimental results: why is only SPENT included in the results table and not the others? I (Markus) do not understand the RMSE on "Training"!? 
	\item I think the main point where reviewers might/will complain is the lack of state-of-the-art baselines. A plain KF seems like an easy baseline, 
	      particularly given the nice discussion on related work earlier on. As a reviewer, I would like to see a more competitive baseline. Not sure though,
		  if this can be done quickly. We can also ignore, submit as is and see what the feedback is. 
\end{itemize}
...
\menz{What about a discussion section, where the experimental results are critically analyzed and future ideas for improvement are outlined? }
...
\begin{table}[htbp]
    %\sffamily %ME: I'd keep that in the regular LaTeX font ...
    \centering
