...
%MD: ich würde bei allen Tu und Tc, jeweils das u oder das c hochstellen. Es ist total ungewöhnlich eine Variable mit ZWEI Charactern abzukürzen
%CH: done. Bild und Texte angepasst.
\begin{figure}[htbp]
...
To guarantee the unambiguity of assigning a \ac{SO} to a single track, each \ac{SO} was synthetically generated at a given time stamp from the pre-existing track set of that time stamp. 
%MD: Hier muss noch ergänzt werden, wie genau die erzeugt wurden!
%CH: die Beschreibung in den folgednen Sätzen ist nicht ausreichend?
...
With $k = 5$ as the number of state values for our work and $n=16$ as the maximum number of tracks per time stamp.
The actual number of tracks can vary between 0 and a maximum of 16 objects, as is given by the KITTI data set using our selected objects (cars and vans) {\cite{Geiger2012CVPR}}. %MD: Add Kitti citation! %CH: done
...
%CH: habe es nochmal geprüft, ich denke es passt so (wie in Teams geschrieben) Powerpoint: alle Formeln in Cambria Math (entspicht italic)
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.95\linewidth]{figs/SANT_network_arch.png}}
	\caption{Schematic representation of the generic network structure of \ac{SANT}.}
	\label{pic::SantArch}
\end{figure}
...
The cross-entropy loss for each prediction, relative to its corresponding target value, is computed as follows in the cost function:
\begin{equation}
    cost = -\frac{1}{N} \sum_{i=1}^{N} \sum_{j=1}^{C} T_{ij} \log(Y_{ij}),
\end{equation}
...
Figure {\ref{pic::MantaData}} shows the input data structure with corresponding association tasks for time stamp 85 of sequence 20 of the KITTI data set.
The respective tracks per time stamp were extracted across all sequences.
...
\begin{strip}
	...
	Seven tracks are extracted from the KITTI data set for the time stamp of sequence 20.
...
The values obtained in this way were arranged as measurements (sensor objects) in a pseudo-random order per time stamp in an $F_{total} \cdot T_{max}$ matrix.
...
Where $F_{total}$ represents the total number of features and $T_{max}$ stands for the maximum number of existing tracks per time stamp.
...
Each track receives a new measurement in this time stamp, and an additional object was detected (new \ac{SO}).
...
\begin{equation}
    loss_t = \sum_{i=1}^{O_{max}} \left( T_{n,i} \ln(Y_{n,i}) + (1 - T_{n,i}) \ln(1 - Y_{n,i}) \right)
\end{equation}
...
\begin{equation}
    cost = -\frac{1}{X} \sum_{x=1}^{S} loss_t
\end{equation}
