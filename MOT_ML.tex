...
We note, however, that genuine hand-labelled \ac{GT} data for this specific association problem is not available. We rather use the existing tracks to  generate synthetical \ac{GT} data for the data association. The input to \ac{SANT} consists of a sensor object $Z_{(t,m=1)}$ and a set of tracks $X_{(t,1:n)}$ (see top part of figure \ref{pic::SantArch}).
The output is a one-hot vector encoding the association of the incoming \ac{SO} to one of the tracks, or to none. To generate the synthetical \ac{GT}, we take a set of tracks, then randomly chose one of them and take the next state vector at the next time stamp of that single track as the incoming new \ac{SO}.
...
The matrix is structured such that the first column contains the state vector of the incoming sensor object $Z_{(t,m=1)}$, while the remaining columns hold the state vectors of the existing tracks $X_{(t,1:n)}$. This matrix is then passed to the \ac{SANT} network for association.
A more detailed analysis of our association data is presented in section \ac{MANTA}.
...
%MD: Bis hier hin habe ich gelesen =================================
