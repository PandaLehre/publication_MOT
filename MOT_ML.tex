...
\begin{abstract}
    ...
    % hier müssen später noch unbedingt die wichtigsten konkreten Ergebnisse rein.
\end{abstract}
...
\cite{KF_simple_cues.2022, KF_simple_online_realtime.2016, DL_CNN_mot_sot_based.2017, DL_RNN_mot.2016, DL_RNN_data_association.2019, DL_ATT_CNN_mot_sot_based.2017, DL_ATT_Trackformer.2022, DL_ATT_CNN_soda.2020}. In complex and dynamic environments, as encountered in urban traffic, it is crucial to
...
\cite{KF_simple_online_realtime.2016, DL_CNN_mot_sot_based.2017, DL_RNN_data_association.2019, DL_RNN_mot.2016}.

\subsection{Tracked object prediction}
%%%%%%%%%%%%%%%%%%%%%% gedanklicher Absatztitel: prediction
One fundamental problem in TbD frameworks is the prediction of the states of the already tracked objects. In many approaches Kalman filters and their variants have proven to be effective for state prediction. %cite, kannst auch etwas nochmal citen
However, they reach their limits in more complex scenarios, particularly in the presence
of non-linear motion patterns and interactions among multiple objects. % finden wir hier auch ein Paper? Grenzen von Kalman filtern, sowas?
...
thus replacing an association algorithm such as the Hungarian Algorithm (HA). %cite for HA algorithm
...
Recurrent Unit (GRU)-based association network can be designed and trained using an undefined distance measure. %MD: Ich finde den nächsten Satz schwer zu verstehen. Kannst du das einfacher umschreiben? Was ist "assignment of temporal memory component"
This can increase the assignment of the temporal memory component, i.e. the history, and thus boost performance.
...
%MD: Ist k als Index besser als t für time? Ich würde unter SANT und SPENT noch association / fusion and prediction schreiben (fusion passiert hier doch auch). Ich würde alle Pfeile beschriften, entweder mit einem Wort daneben oder so wie es hier gemacht wurde mit einer Abkürzung. Unter Delete steht delete_thr, was ist das? Bei den Hiddenstatevectors würde ich oben oder unten in der Beschreibung noch schreiben "one for each track". Warum sind die Pfeile oben dünner als die unten? Steht doch das gleiche drin und warum gibt es unten zwei verschiedene Farben? Steht doch auch das gleiche drin.
\begin{figure}[htbp]
...
% MD: hier gehst du nochmal auf die beiden Modelle einzeln ein, das würde ich nach unten ziehen. Da gibt es doch einen Absatz nur für SPENT und einen nur für SANT. Und alles was eher Einleitungscharakter hat würde ich nach oben in die Introduction ziehen.
...
% MD: Warum brauchst du den nächsten Satz hier? Benutzt du das framework? Wenn ja, würde ich das schreiben. Sonst fehlt der Bezug zur Arbeit.
...
\cite{KF_framework.2013}, for example, provides a framework for analyzing tracking approaches that follow the TbD paradigm.
...
Most existing tracking methods associate incoming detections pairwise with object states predicted by a simple motion model, e.g. a constant velocity model, using a Kalman filter. %cite 3 Artikel
