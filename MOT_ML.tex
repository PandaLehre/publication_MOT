...
\begin{abstract} 
	...
	We propose and evaluate three novel neural networks \ac{NN} designed for \ac{MOT}:
	(i) the \ac{SPENT} for trajectory forecasting, (ii) the \ac{SANT} for mapping single sensor inputs to existing tracks, and (iii) the \ac{MANTa} for associating multiple sensor inputs to multiple tracks.
	...
	In our evaluation, we compare the proposed methods against a traditional \ac{KF} on the public KITTI tracking dataset: \ac{SPENT} reduces the Root Mean Square Error (RMSE) by 50\% on the KITTI tracking dataset compared to a standard KF; SANT and MANTa achieve a 95\% validation accuracy in sensor object assignment to tracks.
	These results underscore the potential of integrating \ac{ML} into tracking systems to enhance performance and robustness, while ensuring the modularity and maintainability of \ac{ADAS} tracking systems, at the same time.
...
%%%%%%%%%%%%%%%%%%% CH: currently commented out due to lack of space
...
\begin{figure}[htbp]
	...
	\caption{Schematic representation of the integration of two neural networks (highlighted in blue) into a \ac{TbD} framework. The association component can be implemented using either the \ac{SANT} or the \ac{MANTa}. These networks work in tandem with the \ac{SPENT} to associate \acp{SO} with tracked objects. \ac{SPENT} predicts object states across time steps, while \ac{SANT}/\ac{MANTa} handle data association. Together, they enable adaptive tracking, overcoming limitations of traditional methods like the \ac{KF} and heuristic-based algorithms.}
...
\ch{UNTIL HERE: I added the acronyms. I will go through the text and add the acronyms where necessary.}
...
\ch{How KF works or how it is implemented in our work?}
...
\ch{Added references to LSTM and BILSTM.}
...
This was done to ensure that the network was trained on tracks with a sufficient number of time stamps. In software development of tracking systems, it is common to use a threshold value to filter tracks that are too short to allow a meaningful prediction. 
...
\ch{Good point. I added a sentence to clarify this.}
...
\ch{I deleted the sentences with the results here and moved them to the results section.}
...
\menz{is it? needs a reference}
\ch{I added references and a sentence to clarify this.}
...
\ch{Scalling the picture down to 0.5 for debugging mode.}
%MD: Auch hier gerne mehr Erklärung in die Unterschrift.
\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.5\linewidth]{figs/MANTa_network_arch.png}}
...
\menz{\begin{itemize}
	...
	\ch{me neither, I would keep / put it in the previous section}
	\item explain the metrics and evaluation methodology
	\item experimental results: why is only SPENT included in the results table and not the others?
	\ch{Added Results for SANT and MANTa in text. Should we add the results in the table as well? Proposal inserted: confusion matrix and values.} 
	\item I (Markus) do not understand the RMSE on "Training"!?
	\ch{Should we delete the training column? and just show the validation and test results?}
	...
	\ch{Based on the small number of training samples, we suggested to use a simple baseline. I am not sure if its possible to implement a more competitive baseline in the short time.}
\end{itemize}}
...
\ch{Confusion matrix must be updated. Just a first suggestion for further discussion.}

\begin{figure}[htbp]
	\centerline{\includegraphics[width=0.95\linewidth]{figs/SANT_confusion_mat.png}}
	\centerline{\includegraphics[width=0.55\linewidth]{figs/SANT_confusion_mat_values.png}}
	\caption{Confusion matrix for the SANT network.}
	\label{pic::SantConfusion}
\end{figure}

\ch{Moved the following part from the MANTa section to the results section.}
...
This means MANTa is trained to assign a list of sensor objects (SOs) to a list of tracks in a single operational step on the same dataset as SANT and achieved an assignment accuracy of 95\% for the six most frequently occurring association sets.
