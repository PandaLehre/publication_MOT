...
For \ac{SANT} we focus on the association of a singular sensor object ($m=1$), denoted as $Z_{(t,m=1)}$, with a collection of $n$ tracks, represented as $X_{(t,1:n)}$.
...
The size of the input matrix therefore corresponds to $k \times (n + 2)$.
...
\begin{equation}
    cost = -\frac{1}{N} \sum_{i=1}^{N} \sum_{j=1}^{C} T_{i,j} \log(Y_{i,j}),
\end{equation}
where $N$ denotes the total number of association samples, $C$ represents the number of class categories, $T_{i,j}$ is the GT indicator for whether class $j$ is the correct classification for sample $i$, and $Y_{i,j}$ is the predicted probability that sample $i$ belongs to class $j$, as derived from the softmax function output.
...
\begin{itemize}
    \item \textbf{1 to n} - one sensor object (SO) with $n$ tracks %(as \ac{SANT})
...
As for SANT, the cross-entropy cost function calculates the cross-entropy loss between network predictions $Y_{i,j}$ and target values $T_{i.j}$ for the unique assignment task for mutually exclusive classes.
...
\begin{equation}
    loss_t = \sum_{i=1}^{O_{\max}} \left( T_{i,j} \ln(Y_{i,j}) + (1 - T_{i,j}) \ln(1 - Y_{i,j}) \right)
\end{equation}
...
\begin{equation}
    cost = -\frac{1}{N} \sum_{x=1}^{N} loss_t
\end{equation}
