...
Both, the Prediction Network and the Association Networks can be developed and evaluated as stand-alone models.
...
% CH: Den letzten Satz habe ich auskommentiert.
...
% CH: Der Punkt ist mir nicht ganz klar. Prediction Network ist der Oberbegriff, SPENT die Abkürzung für single prediction network.
...
and the number of possible assignment classes $C=18$. Where each field can either be zero (no assignment) or one (assignment). For numerical reasons, we unfold this matrix to a one-hot vector of dimension $OH_{max}=288=16\dot 18$.
...
\paragraph{Network architecture}
%MD: Warum architectureS? gibt es mehrere?
