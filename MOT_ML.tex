...
\begin{abstract}
	...
	These results highlight the opportunities of ML integration into tracking systems in enhancing performance
	and robustness while at the same time maintaining
...
(ii) Our Single and (iii) Multi Association Network replaces the calculation of a distance metric for
...
Both, the our Single Prediction Network (SPENT) and our association networks can be developed and evaluated as stand-alone models.
...
In our work, we want to enable the network to follow a completely data-driven association logic
without forcing a concrete distance metric.

\paragraph{Attention Mechanism based Tracker}
In our work we analyze tracks using LSTM based models. An alternative approach would be the use of the attention mechanism \cite{ML_Attention.2017} as e.g. applied by the authors of \cite{DL_ATT_mot_sot_based.2017,
	DL_ATT_CNN_mot_sot_based.2017,
	DL_ATT_CNN_soda.2020,
	DL_CNN_ATT_mot.2017,
	DL_ATT_Trackformer.2022}.
The main research focus of Hung et al. \cite{DL_ATT_CNN_soda.2020} is on soft data association (SoDA),
...
Their approach to SoDA works by using the attention mechanisms
...
Being able to cover these scenarios enables MANTa to adeptly manage the complexities inherent in
...
%MD: Ich bin noch nicht überzeugt, dass man diese Fälle braucht, aber wenn das Netz das jetzt kann okay. Vielleicht nochmal drauf hinweisen, dass wir dadurch das Netz so allgemein und umfassend einsetzbar wie möglich trainieren wollen OBWOHL das natürlich auch über das Framework drum herum gelöst werden kann.
...
association tasks in the displayed time step (85) of sequence 20 of the KITTI data set. %MD DIe infors hier würde ich unbedingt auch unter die Figure schreiben. Die Figure sollte so gut es geht selbsterklärend sein und nicht den Text hier brauchen.
The respective tracks per time step were extracted across all sequences.
The extracted tracks were each modified with noise as in the SANT development and then normalized.
%MD: Warum visual assignment? Du hast doch nichts visually assigned.
...
The values obtained in this way were assigned as shown in Fig. \ref{pic::MantaData} as measurements in pseudo-random order per time step assigned to an $F \cdot T_{max}$ matrix. %MD: Die fig 5 zeigt aber gar kein assignment.
Where $F$ represents the total number of features and $T_{max}$ stands for the maximum number of existing tracks per time step. %MD: Ich würde total aufpassen zwischen number of features (was immer 5 ist) und dem F = 10 hier, was ja nur die Dimension der MAtrix ist. Das verwirrt sonst. F würde ich vlt "total features" nennen.
The total number of features results from the status values per track and SO set.
(here, $F = 2 \cdot m$, with $m=5$ being the number of state values in this investigation).
...
The GT assignment is displayed at the bottom of the section. The assignment output can be thought of a matrix with dimensions maximum number of tracks $T_{max}=16$
and the number of possible assignment classes $C=18$. Where each field can either be zero (no assignment) or one (assignment). For numerical reasons, we unfold this matrix to a one-hot vector of dimension $OH_{max}=288=16 \cdot 18$.
...
\begin{strip} % <--- defined in "cuted"
    ...
    %MD Hier bitte noch mehr Erklärung: Was genau zeigt der one-hot vektor? Und auch schreiben, dass oben 7 tracks sind und unten 8 SOs. Und die Infos aus dem Text hier aufnehmen.
	\label{pic::MantaData}
\end{strip}
...
\paragraph{Network architecture}
The schematic representation \ref{pic::MantaArch} shows the developed network architecture
...
% Außerdem steht in der Abbildung X_ und Z_ und die Grammatik ist falsch ("a mutiple ...")... Auch würde ich X nochmal benennen.
\begin{figure}[htbp]
...
%MD: Die Bezeichnungen in der Abbildung und die im Text müssen exakt gleich sein. Hier schreibst du BILSTM layer in der Abbildung steht "biLSTM_last". Ich würde die Bezeichnung in der Abbildung ändern.
...
This extension is labelled accordingly in figure \ref{pic::MantaArch}. %MD: Check if next sentence is correctly rewritten by me. "familiar" war zu vage.
For each track (from 1 to $T_{max}=16$), the MANTa has been developed with
the fully connected, softmax stack that was introduced for SANT.
Each softmax output consists of a vector with $C=18$ elements,
which represents the most probable assignment.
This means that a single assignment can be realized for each track. %MD Auch hier: Namen MÜSSEN exakt übereinstimmen "Concatenation Layer" gibt es nicht in der Zeichnung.
...
the most probable assignment of a measurement to a track. %MD: Was heißt according to the format? das würde ich umformulieren. Ich mache ja keine Kosten wegen einem Format.
The cost function was implemented according to the format of the GT and output data of the network. %MD: Den nächsten Satz verstehe ich nicht. "clear assignment", "additional iteration", "respective track blocks". Bitte alles klarer machen.
The cross-entropy loss function already introduced was used and a clear assignment was realized
...
input value $Y_{i}$ and associated target value $T_{i}$ element by element at time step $i$. %MD: i ist der Time step, richtig?
%MD: Warum k? Wofür steht das. Das muss in den Text.
To obtain a scalar $loss_k$ per time step, all loss values are summed up and
divided by the number of classes $C=18$.
This results in the average loss per time step for all tracks.
\begin{equation}
	loss_k = \frac{1}{C} \sum_{i=1}^{C} -(T_i ln(Y_i) + (1 - T_i) ln(1 - Y_i))
\end{equation}
Then, all scalars obtained per time step are summarized and divided by the number of samples $N$ of a minibatch: %MD: Diese Summe hat keinen Index, der muss dazu. Ich gehe mal davon aus, dass es k ist und k die ANzahl an Samples in einer Minibatch ist.
\begin{equation}
...
occurring association sets. %MD: Was sind die "most frequently occuring association sets? --> Klarer
%MD: Wenn wir schon schreiben following section, dann kann die ruhig einen Titel kriegen
...
%MD: Warum allocation? Warum nicht association?
...
\begin{figure}[htbp] %MD: Die rechte Seite der Grafik ist zu klein. Alle Zahlen müssen größer. Bitte nicht einfach die Grafik größer machen, wir haben sonst zu wenig Platz.
...
%MD: Was ist der Unterschied zwischen Tracking-by-Detection Framework und Multi-Object Tracking process? Wahrscheinlich wenig. Daher würde ich mindestens ein von den beiden hier aus dem Satz raus schmeißen.
...
%MD: ALles wird hier nochmal ausgeschrieben, SPENT aber nicht.
...
of a dynamics model.
Our implementation uses a recurrent network to update its internal
...
Network verification demonstrates a Root Mean Square Error (RMSE) of 0.026, %MD: Das hat keine Einheit oder doch?
averaged over the training, validation, and test datasets. %MD: Wirklich über alle? Das können wir nicht machen. Das darf nur auf dem test set angegeben werden. Sollten wir nochmal drüber reden.
...
%MD: Den nächsten Absatz würde ich nochmal überarbeiten. Der wirkt noch holprig und wenig schlüsslig. Was wird denn ersetzt, GNN oder HA oder beides? Und ich würde immer schreiben "such as", es gibt ja genug KF mit anderen Ass. Methoden, die können aber auch SANT oder MANTa nutzen.
...
We would like to express our sincere thanks to the organizations that provided
financial support for this research project, namely the Daimler Truck AG and the Baden-Württemberg Cooperative State University (DHBW) Stuttgart.
