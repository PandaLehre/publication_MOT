...
\begin{abstract}
% MD: old abstract. I wanted to improve the reading flow and reduce repetition.
	...
	% (i) the \ac{SPENT} for trajectory prediction, (ii) the \ac{SANT} for mapping a single \ac{SO} to an existing track, and (iii) the \ac{MANTa} for associating multiple \acp{SO} to multiple tracks.
	% These \ac{ML} models are integrated into a traditional \ac{KF} framework in a way the preserves the system's modularity, as we replace the respective components.

	% In our evaluation, we compare the proposed methods against the traditional \ac{KF} on the public KITTI tracking dataset: \ac{SPENT} reduces the Root Mean Square Error (RMSE) by 50\% compared to a standard KF, SANT and MANTa achieve a 95\% validation accuracy in sensor object assignment to tracks.
	% These results highlight the potential of integrating task-specific \ac{NN}s into tracking systems to enhance performance and robustness, while ensuring modularity, maintainability and interpretability at the same time.
...
In complex and dynamic environments, as encountered in urban traffic, it is crucial to accurately capture and predict the positions of multiple objects already in the early timesteps of detection -- a key challenge in automated driving.
...
%2nd version
%%%%%%%%%%%% CH: shorter version %%%%%%%%%%%%%
% In the automotive industry, tracking systems are usually developed as platform software for various vehicle models, each with different sensors positions, system configurations or even different sensor sets. These systems must operate reliably across diverse driving scenarios, but this can lead to performance issues in specific cases. Traditional approaches frequently address these problems with heuristics and parameter adjustments, which makes the software difficult to maintain and extend. Hand-engineered methods lack automated ways of optimization, which leads to suboptimal performance in in complex driving situations. We propose a data-driven tracking framework that allows fine-tuning for specific configurations, improving both maintainability and adaptability.
...
%MD:
% - Delete -> Deletion (sollten beides Substantive sein)
% - Groß und Kleinschreibung nicht einheitlich: Was sind die Regeln hier? Zumindest unten Sensor Objects, das O groß. Das wird zwei mal verwendet. Auch muss es "Assignment of sensor objectS" 1:m heißen.
% - ich würde bei allen Tu und Tc, jeweils das u oder das c hochstellen. Es ist total ungewöhnlich eine Variable mit ZWEI Charactern abzukürzen.
\begin{figure}[htbp]
...
\ac{SPENT} predicts all vectors $X_{t,1:n}$ to the next time step where they are used as input to either the SANT or MANTa association networks.
...
The mean values $\vec{\mu}$ and standard deviations $\vec{\sigma}$ for each state in the state vectors $\vec{X}_t$ were computed across all tracks. For this, all tracks were joined together, leading to one pseudo-track with total number of time stamps $m$:

\begin{equation}
	\vec{\mu} = \frac{1}{m} \sum_{t = 1}^{m} \vec{X}_t
\quad\text{and}\quad
	\vec{\sigma} = \sqrt{{\frac{1}{m - 1}} \sum_{t = 1}^{m} (\vec{X}_t - \vec{\mu})^{2^*}},
\end{equation}
...
\end{equation}
where $k$ is the length of the predicted state vector (here $k=5$), $\vec{Y}$ is the ground truth state vector (KITTI cars and vans tracks) and $\vec{\hat{Y}}$ is the predicted state vector from our network.
The cost function is evaluated for several sequences, each with numerous time stamps.
For our prediction network the cost function is half the mean-square-error of the predictions added up for each time stamp in the training set, normalized by the total number of all time stamps $T$ in the used sequences.
The factor of $\frac{1}{2}$ simplifies the gradient during backpropagation:
\begin{equation}
	cost = \frac{1}{2}\frac{1}{T} \sum_{t=1}^{T}\frac{1}{k}\sum_{i=1}^{k}{({Y_{(t),i}}-{\hat{Y}_{(t),i}})}^2,
\end{equation}
where $Y_{(t),i}$ refers to the $i$-th entry in the state vector at time stamp $t$. During training, the average cost is calculated using the observations in the mini-batch, so $T$ is given by the mini-batch length.
...
For \ac{SANT} we focus on the association of a singular sensor object, denoted as $Z_{(t,1)}$, with a collection of tracks, represented as $X_{(t,1:n)}$.
These tracks were carefully curated from the KITTI dataset, which comprises annotated camera recordings.
We note, however, that genuine \ac{GT} data for the specific association problem at hand is not available.
To guarantee the unambiguity of assigning a \ac{SO} to a single track, each \ac{SO} was synthetically generated at a given timestep from the pre-existing track set of that timestep. %MD: Hier muss noch ergänzt werden, wie genau die erzeugt wurden!
...
To achieve this, we add noise with a maximum of 3\% to each of the current state vector values.
...
The size of the input matrix therefore corresponds to $k \times n + 1$.
With $k = 5$ as the number of state values for our work and $n=16$ as the maximum number of tracks per time step.
The actual number of tracks can vary between 0 and a maximum of 16 objects, as is given by the KITTI data set using our selected objects (cars and vans). %MD: Add Kitti citation!
...
At each time step, a matrix is passed as input holding both, the information of the one to-be-assigned \ac{SO} and the multiple currently-tracked objects.

%MD: da wir m unten nutzen für die verschiedenen Assoziationen und du oben k als Anzahl an Einträge für den state Vector eingefügt hast, muss bei Z_t,m das m mit einem k ersetzt werden. Dazu ist Z noch nicht italic, im Text ist es überall italic.
\begin{figure}[htbp]
...
The softmax function converts all $z_i$ values into a probability vector with $i$ values.
