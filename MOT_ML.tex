...
%CH: Bitte prüfen, ob die groß und kleinschreibung so korrekt ist
...
Fig. {\ref{pic::MotFramework}} provides an overview of our approach in which the association network can be implemented using either \ac{SANT} or \ac{MANTa}.
...
To generate the synthetical \ac{GT}, we take a set of tracks, then randomly chose one of them and take the next state vector at the next time stamp of that single track as the incoming new \ac{SO}.
...
Fig. {\ref{pic::MantaData}} shows the input data structure with corresponding association tasks for time stamp 85 of sequence 20 of the KITTI data set. Equivalent data points were extracted across all sequences.
...
Each sensor object is given a $1 \times 18$ one-hot vector as its association result. 
For a maximum of $T_{\max}=16$ tracks, this results in an output vector of size $16 \times 18 = 288$. 
...
This approach, well-suited for real-time applications with minimal latency, presents a strong alternative to traditional methods. 
...
Our model achieved a \ac{RMSE} of 0.029 on the testing set for positional predictions across 31 tracks (Table {\ref{tab::SpentRmseComparison}}), with average deviations of 0.42 meters on the X-axis and 0.23 meters on the Y-axis
...
For another primary task of the \ac{TbD} \ac{MOT} method, namely the association of sensor objects to existing tracks, we developed \ac{SANT} to replace the classical \ac{GNN} method. By substituting the distance metric and the Hungarian assignment procedure with a learned, data-driven assignment logic, \ac{SANT} achieves an impressive accuracy of 95\% on a testing dataset with approximately 391 samples (representing 5\% of the total dataset with 7827 association samples). This approach not only simplifies the assignment process but also allows for adaptability in diverse scenarios, where classical methods may lack flexibility.
...
In this paper, we introduced machine learning methodologies that enhance \ac{MOT} for \ac{ADAS} at the object list level. 
This work lays a foundation for future \ac{ADAS} research, highlighting the potential of data-driven software development in overcoming the limitations of classical heuristic algorithms. 
Furthermore, we also plan to test the models on other datasets, such as the nuScenes dataset, to evaluate their generalization capabilities.
Future research directions include the quantification of uncertainties to improve decision making and the investigation of multitasking networks to optimise the context-dependent tracking of multiple objects.
