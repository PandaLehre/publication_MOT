...
\title{Enhancing and Simplifying Multi-Object Tracking with Data-Driven Prediction and Association Techniques}
...
\cite{KF_Bewley.2016,KF_simple_cues.2022,KF_BoundingBD.2023,OD_kampker.2018,OD_wu.2022}.
...
and in the automotive industry tracking systems are not developed for a single model,
...
Both, the our Single Prediction Network and our association networks can be developed and evaluated as stand-alone models.
...
% CH: Verweis auf die Ergebnisstable in der Conclusion eingefügt.
...
within the test dataset, representing 31 Tracks (see Table \ref{tab::SpentRmseComparison}).
This performance metric translates to an average deviation of 0.42 meters for predictions
pertaining to the X coordinate and 0.23 meters for those related to the Y coordinate.
...
%		Es werden alle Tracks pro Zeitschritt über das BILSTM betrachtet.
...
%CH: Gibt der Text das so nicht wieder? 
...
The training, validation and test data set of MANTa was created according to
the described objective.
Figure \ref{pic::MantaData} shows the input data structure with corresponding
association tasks for time step 85 of sequence 20 of the KITTI data set. 
...
%CH: Showcase für den Leser, wie die Datenstruktur aussieht. --> example assignment?
Figure \ref{pic::MantaData} shows the non-noisy sensor objects to enable
a example assignment and increase understanding of the association procedure.
%MD: Die fig 5 zeigt aber gar kein assignment.
%CH: Stimmt, der one-hot Vektor zeigt das Assignment :) 
...
The cross-entropy cost function calculates the cross-entropy loss between network predictions $Y_{k,i}$ and target values $T_{k,i}$ for the unique assignment task for mutually exclusive classes. 
...
%CH: Habe ich so eingefügt.
MANTa is trained to assign a list of sensor objects (SOs) to a list of tracks in a single operational step.
MANTa was trained on the same dataset as SANT and achieved an assignment accuracy of 95\% for the six most frequently occurring association sets.
Thus are sets with one to six tracks per time step. 
%MD: Die rechte Seite der Grafik ist zu klein. Alle Zahlen müssen größer. 
%CH: Ich habe die Grafik und caption noch einmal angepasst.
\begin{figure}[htbp]
...
%%%%%%%%%%%%%%%%%%%%%% CH: bis hier habe ich überarbeitet. Grafik und Text muss ich noch überarbeitn. %%%%%%%%%%%%%%%%%%%%%%
...
%MD: Check if next sentence is correctly rewritten by me. "familiar" war zu vage.
...
%MD Auch hier: Namen MÜSSEN exakt übereinstimmen "Concatenation Layer" gibt es nicht in der Zeichnung.
...
%MD: Was heißt according to the format? das würde ich umformulieren. Ich mache ja keine Kosten wegen einem Format.
...
framework, effectively replacing classical heuristic algorithms.
...
%MD: Den nächsten Absatz würde ich nochmal überarbeiten. 
%    Der wirkt noch holprig und wenig schlüsslig.
%    Was wird denn ersetzt, GNN oder HA oder beides? Und ich würde immer schreiben "such as",
%    es gibt ja genug KF mit anderen Ass. Methoden, die können aber auch SANT oder MANTa nutzen.
...
financial support for this research project, namely the Daimler Truck AG
and the Baden-Württemberg Cooperative State University (DHBW) Stuttgart.
